...
export const searchProducts = async (search: string): Promise<IProduct[]> => {
  const products = await getResource(
    "shop/products/search/{search}/{locale}",
    {
      search: search,
      locale: getLocale(),
    },
...
export const getProductByTranslatedSlug = async (
  slug: string
): Promise<IProduct> => {
  const product = await getResource(
    "shop/products/by-slug/{slug}/{locale}",
    {
      slug: slug,
      locale: getLocale(),
    },
...
export const getProductsByTaxon = async (
  taxon: ITaxon | null
